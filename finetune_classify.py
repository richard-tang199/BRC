...
from model.one_to_all import PatchClassify, Patch_RWKV, PatchFrequency
from toolkit.utils import *
from toolkit.get_retraining import get_retrain_windows, RetrainDataset
import json
import random
...
parser.add_argument('--model_name', type=str, default='retrain', help='Name of the model')
parser.add_argument('--dataset_name', type=str, default='UCR', help='Name of the dataset')
parser.add_argument("--group_name", type=str, default=7, help="group in the dataset")
...
parser.add_argument("--finetune_steps", type=int, default=10, help="number of finetune steps for training")
...
if __name__ == '__main__':
    ...
    for i, (test_data, test_label, subsequence_length) in enumerate(
        ...
        os.makedirs(f"new_results/group_{args.group_name[i]}", exist_ok=True)
    ...
    windows, window_labels = get_retrain_windows(
        test_data=test_data,
        test_anomaly_scores=test_anomaly_score,
        window_length=window_length,
        test_labels=test_label
    )


    retrain_dataset = RetrainDataset(windows, window_labels)
    retrain_loader = DataLoader(retrain_dataset, batch_size=args.batch_size, shuffle=True)

    for step in range(args.finetune_steps):
        model.train()
        epoch_loss = 0
        for data, label in retrain_loader:
            data = data.to(device)
            loss = model(data, if_finetune = False)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            epoch_loss += loss.item()
        print(f"Epoch {epoch}: Loss {epoch_loss / len(retrain_loader)}")
