...
class AnomalyScoreCalculator:
    ...
    def get_error_score(self,
        ...
        if self.average_window is not None:
            ...
            average_train_score_normalized = np.convolve(train_score_normalized, weight, mode='same')
            average_test_score_normalized = np.convolve(test_score_normalized, weight, mode='same')

            if self.add_average:
                train_score_normalized = (average_train_score_normalized + train_score_normalized) / 2
                test_score_normalized = (average_test_score_normalized + test_score_normalized) / 2
            else:
                train_score_normalized = average_train_score_normalized
                test_score_normalized = average_test_score_normalized
