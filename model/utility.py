...
class MaskingStrategy(nn.Module):
    ...
    def __init__(self,
        ...
        if mode not in ["random", "gating", "switching"]:
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.mask is None:
            if self.mode == "gating":
                ...
                batch_size, num_channels, sequence_length, num_features = inputs.shape
                index_value = torch.rand(size=(batch_size, num_channels, sequence_length),device=inputs.device)
                index_value = index_value.reshape(batch_size, num_channels, -1, 2)
                index_value = index_value > index_value.mean(dim=-1, keepdim=True)
                index_value = index_value.reshape(batch_size, num_channels, sequence_length)

                odd_even_masking = index_value.long()
                even_odd_masking = (odd_even_masking == 0).long()
                odd_even_masking = odd_even_masking.unsqueeze(-1).repeat(1, 1, 1, num_features)
                even_odd_masking = even_odd_masking.unsqueeze(-1).repeat(1, 1, 1, num_features)
                inputs_mask_odd_even = inputs.masked_fill(odd_even_masking.bool(), self.mask_value)
                inputs_mask_even_odd = inputs.masked_fill(even_odd_masking.bool(), self.mask_value)
            else:
