...
parser.add_argument('--model_name', type=str, default='dada', help='Name of the model')
...
if __name__ == '__main__':
    ...
    if args.model_name == "cutaddpaste":
        ...
        model_configs = {'sequence_length': 64, 'stride': train_stride}
        ...
        start_time = time.time()
        test_anomaly_score, flops, params = model.zero_shot(test_loader)
        duration = time.time() - start_time
        ...
        model.fit(train_data)
        test_anomaly_score = model.decision_function(test_data)
        eval_result = evaluate(ground_truth=test_label,
                               anomaly_scores=test_anomaly_score,
                               subsequence=subsequence_length,
                               use_ucr=use_ucr,
