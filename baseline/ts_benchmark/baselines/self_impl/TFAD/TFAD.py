...
from baseline.ts_benchmark.baselines.self_impl.TFAD.TFAD_model import TFADModel
from baseline.ts_benchmark.baselines.self_impl.TFAD.TSDataset import TimeSeries
from baseline.ts_benchmark.baselines.self_impl.TFAD import transforms
from baseline.ts_benchmark.baselines.self_impl.TFAD.model.tfad_datamodule import reduce_labels
from baseline.ts_benchmark.baselines.time_series_library.utils.tools import (
    EarlyStopping,
    adjust_learning_rate,
)
from baseline.ts_benchmark.baselines.utils import (
    train_val_split,
...
class TFAD:
    ...
    def detect_fit(self, train_data: pd.DataFrame):
        """
        训练模型。
        ...
        Time_series_temp = TimeSeries(train_data.values)
    ...
    def _detect_score(
        ...
        # flops, params = thop.profile(self.model, inputs=(ts_windows_chunk,))
        # self.flops = flops
        # self.params = params
