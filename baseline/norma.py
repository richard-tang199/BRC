...
class NORMA():
    def __init__(self, pattern_length, nm_size, clustering='hierarchical', percentage_sel=0.4, overlapping_factor=1,
                 sampling_division=10, number_of_cluster=10):
    ...
    def run(self, ts):
        ...
        join_n = np.array(
            [join_n[0]] * (self.pattern_length // 2) + list(join_n) + [join_n[-1]] * (self.pattern_length // 2))
...
def cutting_method(recurrent_sequence, correlation_matrix, dendo, distances,
    ...
    if cut_method == "max":
        ...
        listcluster = fcluster(dendo, (cluster_level * (distances.max() - distances.min())) + distances.min(),
                               'distance')
        ...
        DdlClusters, centerWithMinDl, chosenCluster, sumDl, DicBitSavedForCluster, totalBitSaved = returnClustersMDL_AndSumMDL(
            recurrent_sequence, listClusterReturn)
        ...
        while (level >= end):
            ...
            if (lastNumberCluster < nunmberOfCluster):
                DdlClusters, centerWithMinDl, chosenCluster, sumDl, DicBitSavedForClusterAc, totalBitSavedAc = returnClustersMDL_AndSumMDL(
                    recurrent_sequence, listclusterActual)
                lastNumberCluster = nunmberOfCluster
...
def clustering_method(recurrent_sequence, corr_method="pearson", linkage_method="complete", cut_method="max",
                      cluster_level=0.33,
                      metric='euclidean'):  #this metric is the default scipy metric for the used functions
    """
        INPUT
            recurrent_sequence: A dataframe containg the list of time series in the columns to cluster
            corr_method: The correlation method to use in order to create the dendogram used to cluster
...
def choose_normalmodel(listcluster, recurrent_sequence, sequence_rec):
    ...
    for weight, min_max, c_mean in zip(score_weight, min_max_diff, cluster_mean_diff):
        ...
        min_max_n = (float(min_max - np.min(min_max_diff)) / float(
            np.max(min_max_diff) - np.min(min_max_diff) + 1)) + 1.0
        #dist_n = (float(dist - np.min(score_distribution))/float(np.max(score_distribution)-np.min(score_distribution)+1))+1.0
        c_mean_n = (float(c_mean - np.min(cluster_mean_diff)) / float(
            np.max(cluster_mean_diff) - np.min(cluster_mean_diff) + 1)) + 1.0
